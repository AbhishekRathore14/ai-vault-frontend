  ...
  return (
    <div className="min-h-screen bg-[#fafbfc] py-12 px-4 text-slate-900 font-sans selection:bg-blue-200">
      <main className="max-w-4xl mx-auto space-y-6">
        ...
        {/* PREMIUM HEADER */}
        <header className="flex flex-col sm:flex-row justify-between items-center bg-white p-6 rounded-[2rem] shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100">
          ...
          <Link href="/new" className="group bg-slate-900 text-white px-7 py-3 rounded-2xl font-bold shadow-lg shadow-slate-200 hover:bg-blue-600 hover:shadow-blue-200/50 transition-all active:scale-95 flex items-center gap-2 mt-4 sm:mt-0">
        ...
        {/* SEARCH BAR */}
        <div className="bg-white/80 backdrop-blur-md p-2 rounded-2xl shadow-sm border border-slate-200/60 flex items-center gap-3 px-5 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all">
        ...
        <div className="space-y-4 pt-2">
          ...
          {/* CONDITION 1: Loading Spinner */}
          {isLoading ? (
            <div className="flex flex-col items-center justify-center p-12 text-center mt-10">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mb-4"></div>
              <p className="text-lg font-medium text-slate-700">Waking up the secure database...</p>
              <p className="text-sm text-slate-500 mt-2">(Free tier servers may take up to 20 seconds)</p>
            </div>
          ) : 
          
          /* CONDITION 2: Empty Vault */
          filteredNotes.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-[2rem] border border-slate-100 shadow-sm">
              ...
              <Link href="/new" className="mt-4 inline-block font-medium text-blue-600 hover:text-blue-700 hover:underline">
                + Add your first note
              </Link>
            </div>
          ) : 
          
          /* CONDITION 3: Show Notes Grid */
          (
            <div className="grid grid-cols-1 gap-4">
              {filteredNotes.map((note) => (
                <div key={note._id} className="group bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-100 hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all flex flex-col sm:flex-row justify-between sm:items-center gap-4">
